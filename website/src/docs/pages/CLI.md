...
Release builds report the released package version, such as `mesh-llm 0.76.0-rc4`.
Local source builds may include build metadata, such as `mesh-llm 0.76.0-rc4+gABCDEF.dirty`, so you can tell exactly which commit produced the binary. Compatibility checks, native-runtime cache paths, and release identity still use the plain release version.
